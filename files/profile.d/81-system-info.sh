...
if [[ ! -e '/usr/bin/curl' && ! -e '/usr/bin/wget' ]]; then
    echo "Error: Neither curl nor wget command was found. You must install curl or wget first."
    exit 1
fi
...
ips=$( /sbin/ip addr show | awk '$1 == "inet" {gsub(/\/.*$/, "", $2); print $2}' | egrep -v "127.0.0.1" | xargs )
...
echo -e "CPU frequency        : ${BLUE}$cpu_freq MHz${PLAIN}"
echo -e "CPU usage            : ${BLUE}$cpu_usage${PLAIN}"
